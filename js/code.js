    ...
    xhr.onreadystatechange = function () {
      if (this.readyState === 4 && this.status === 200) {
        ...
        list.innerHTML = results.map(c => {
          const fn = c.FirstName ?? c.firstName ?? "";
          const ln = c.LastName  ?? c.lastName ?? "";
          return `<div class="autofill-item" onclick="selectSearch('${jsSafe(fn)} ${jsSafe(ln)}')">
                    ${escapeHtml(fn)} ${escapeHtml(ln)}
                  </div>`;
        }).join("");
      }
    };
...
function selectSearch(name) {
  ...
  // Optionally ping server with the chosen name too
  fetchContacts(name, true);
}
